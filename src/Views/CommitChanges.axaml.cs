...
namespace SourceGit.Views
{
    public partial class CommitChanges : UserControl
    {
        ...
        {
            if (DataContext is not ViewModels.CommitDetail vm ||
                sender is not ChangeCollectionView { Selection: { Count: 1, IsSingleFolder: false } selection } view)
                return;

            var cmdKey = OperatingSystem.IsMacOS() ? KeyModifiers.Meta : KeyModifiers.Control;
            if (view.LastDoubleTappedKeyModifiers.HasFlag(cmdKey))
                vm.OpenChangeInMergeTool(selection.Changes[0]);
            else
                vm.OpenChangeInDiffWindow(selection.Changes[0]);

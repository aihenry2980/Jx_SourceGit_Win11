...
namespace SourceGit.Views
{
    public partial class RepositoryToolbar : UserControl
    {
        ...
        private void AddToolbarGitCommandEditorMenuItems(ContextMenu menu, ViewModels.Repository repo, ToolbarGitButtonKind kind)
        {
            ...
            if (kind is ToolbarGitButtonKind.SyncAll)
        ...
        private ToolbarGitCommandSpec BuildPullCommandSpec(ViewModels.Repository repo)
        {
            ...
            builder.AppendLine("# Pull only the top repository; submodule recursion is explicitly disabled.");
            builder.Append("git pull --verbose --progress --no-recurse-submodules --no-rebase ")
            ...
            return new ToolbarGitCommandSpec(
                "Edit command...",
                "Edit Pull Top Repository Command",
                "Edit the direct pull command for the checked-out branch in the top repository. Submodules are not touched.",
                builder.ToString(),
        ...
        private async Task<ToolbarGitCommandSpec> BuildFetchRecursivelyCommandSpecAsync(ViewModels.Repository repo, bool alternateMode)
        {
            ...
            {
                builder.Append("git fetch --progress --verbose --no-tags --prune --no-recurse-submodules ")
                    .Append(Quote(remote))
                    .AppendLine();
            }

            var submodules = await new Commands.QuerySubmodules(repo.FullPath, int.MaxValue, false).GetResultAsync();
            foreach (var submodule in submodules)
            {
                var submoduleRoot = Native.OS.GetAbsPath(repo.FullPath, submodule.Path).Replace('\\', '/');
                ...
                foreach (var remote in remotes)
                {
                    builder.Append("git -C ")
                        ...
                        .Append(" fetch --progress --verbose --no-tags --prune --no-recurse-submodules ")
                        .Append(Quote(remote))
                        .AppendLine();
                }
            }
            ...
            return new ToolbarGitCommandSpec(
                "Edit command...",
                "Edit Fetch + Prune All Repositories Command",
                "Edit the sequential fetch and prune commands for the top repository and every initialized nested submodule.",
        ...
        private async void PullTopRepository(object sender, TappedEventArgs e)
        {
            if (DataContext is ViewModels.Repository repo)
            {
                e.Handled = true;
